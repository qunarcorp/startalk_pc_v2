...
public:
    ...
    virtual bool mouseReleaseEvent(QMouseEvent *, QWidget* parent = nullptr, bool forcesave=false);
    ...
    QRect TargetRect() const {
        ...
        QRect val;
        val.setX(m_targetRect.x() * m_Radio);
        val.setY(m_targetRect.y() * m_Radio);
        val.setWidth(m_targetRect.width() * m_Radio);
        val.setHeight(m_targetRect.height() * m_Radio);
        return val;
    }
    void setTargetRect(QRect val) {
        m_targetRect.setX(val.x() / m_Radio);
        m_targetRect.setY(val.y() / m_Radio);
        m_targetRect.setWidth(val.width() / m_Radio);
        m_targetRect.setHeight(val.height() / m_Radio);
    }
