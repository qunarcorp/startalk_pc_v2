...
FullScreenWidget::FullScreenWidget() {
    m_d.setDevicePixelRatio(this->devicePixelRatio());
    setMouseTracking(true);
    using namespace Qt;
    auto remove = WindowTitleHint;
    auto add = Qt::FramelessWindowHint | Qt::WindowMinMaxButtonsHint | Qt::Tool | Qt::WindowStaysOnTopHint |
...
void FullScreenWidget::loadBackgroundPixmap() {
    ...
    QRect deskRect = deskTop->availableGeometry(QCursor::pos());

    int width = deskRect.width();
    int height = deskRect.height();

    screenx = deskRect.left();
    screeny = deskRect.top();

    screenwidth = width;
    screenheight = height;
    m_DesktopRect = deskRect;
...
#endif
#ifdef _MACOS
    QPixmap result = QPixmap::grabWindow(QApplication::desktop()->winId(), screenx, screeny, width, height);
    loadBackgroundPixmap(result, screenx, screeny, width, height);
...
void FullScreenWidget::loadBackgroundCursorPixmap() {
    ...
    QRect pScreenRect = QApplication::desktop()->availableGeometry(QCursor::pos());
    ...
    if (pScreenRect.contains(pt)) {
        ...
        screenx = nleftPos;
        screeny = ntopPos;
        width = pScreenRect.width() * QApplication::desktop()->devicePixelRatio();
        height = pScreenRect.height() * QApplication::desktop()->devicePixelRatio();
    }
...
QPixmap FullScreenWidget::getFullScreenPixmap() {
    ...
    m_DesktopRect = QApplication::desktop()->availableGeometry(QCursor::pos());
    result = QPixmap::grabWindow(QApplication::desktop()->winId(), screenx, screeny, m_DesktopRect.width(),
                                 m_DesktopRect.height()); //抓取当前屏幕的图片
    return result;
}
...
void FullScreenWidget::checkMoveEndPoint() {
    ...
    if (x + selectedRect.x() < screenx) { //当移动后X坐标小于零时，则出现选区丢失，则计算出moveEndPoint的X最大坐标值，进行赋值
        moveEndPoint.setX(qAbs(selectedRect.x() - moveBeginPoint.x()));
    }

    if (y + selectedRect.y() < screeny) { //当移动后Y坐标小于零时，则出现选区丢失，则计算出moveEndPoint的Y最大坐标值，进行赋值
        moveEndPoint.setY(qAbs(selectedRect.y() - moveBeginPoint.y()));
    }

    if (x + bottomRightPoint.x() > screenwidth * this->devicePixelRatio()) { //当移动选区后，出现超出整个屏幕的右面时，设置moveEndPoint的X的最大坐标
        moveEndPoint.setX(screenwidth *this->devicePixelRatio() - bottomRightPoint.x() + moveBeginPoint.x());
    }

    if (y + bottomRightPoint.y() > screenheight * this->devicePixelRatio()) { //当移动选区后，出现超出整个屏幕的下面时，设置moveEndPoint的Y的最大坐标值
...
QRect FullScreenWidget::getMoveAllSelectedRect() {
    QRect result;
...
QRect FullScreenWidget::getMoveControlSelectedRect() {
    int x, y, w, h;
...
QRect FullScreenWidget::GetCursorInRect() {
    ...
    QPoint newglcurser = glCurser * nRatio;
    QRect glRec = m_d.GetTheTopRect(newglcurser);
...
void FullScreenWidget::drawHoverWindowPixmap() {
    QRect rec = GetCursorInRect();
    static QRect sLastRec;
    if (rec != sLastRec) {
        sLastRec = rec;
        sLastRec.setTopLeft(QPoint(sLastRec.x(), sLastRec.y()));
        m_selectedRect = sLastRec;
        beginPoint = sLastRec.topLeft();
        m_endPoint = sLastRec.bottomRight();
    ...
    shotPixmap = loadPixmap.copy(rectnew);  //更新选区的Pixmap
...
void FullScreenWidget::drawSelectedPixmap() {
...
void FullScreenWidget::drawXYWHInfo() {
    int x, y;
    ...
    switch (currentShotState) {
        ...
        case lbclick: {
            ...
            y = m_selectedRect.y() / nRatio > (infoHeight + 5) ? (m_selectedRect.y() / nRatio - infoHeight - 3) :
                m_selectedRect.y() / nRatio + 3;
            ...
            strTipsText = QStringLiteral(" 区域:%1 x %2 \n ESC 取消截图\n 鼠标双击完成截图").arg(m_selectedRect.width(), 4).arg(
...
#ifdef _MACOS
//    QStringList args;
//    QString tmpFile = "/tmp/1.png";
//    args << "-i" << tmpFile;
//    QProcess::execute("/usr/sbin/screencapture", args);
//    const char *command = "/usr/sbin/screencapture";
//
//    const char *arguments[] = {"-ic"/*, "/tmp/1.png"*/};
//    MacHelper::runCommand(command, (char **) arguments, 1);

//    if (!QFile::exists(tmpFile)) {
//        return;
//    }

//    QPixmap result(tmpFile);
//    QApplication::clipboard()->setImage(result.toImage());
//    auto *tool = qobject_cast<SnipScreenTool *>(this);
//    if (tool) emit tool->sgFinish(tool->conversionId());
 ...
 *	鼠标停留的信息预览框
 */
void FullScreenWidget::drawShotPictureInfo() {
    ...
    int wholeRectX =
            frameCurser.x() + 147 > screenwidth / this->devicePixelRatio() ? frameCurser.x() - 137 : frameCurser.x() +
                                                                                                     15;
    int wholeRextY =
            frameCurser.y() + 145 > screenheight / this->devicePixelRatio() ? frameCurser.y() - 135 : frameCurser.y() +
                                                                                                      15;
    ...
    painter.drawLine(QPoint(rec2ndBorder.x() + 1, rec2ndBorder.y() + 39),
                     QPoint(rec2ndBorder.x() + 117, rec2ndBorder.y() + 39));
    painter.drawLine(QPoint(rec2ndBorder.x() + 59, rec2ndBorder.y() + 1),
