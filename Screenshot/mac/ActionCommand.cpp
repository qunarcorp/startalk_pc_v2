...
void ActionCommand::mouseMoveEvent(QMouseEvent * e)
{
    ...
    if (_drawState==drawBegin||_drawState==drawGoing)
    {
        ...
        do 
        {
            if (p.x() < m_targetRect.left()) {x = m_targetRect.left();break;}
            if (p.x() > m_targetRect.right()) {x = m_targetRect.right();break;}
            x = p.x();
        } while (0);

        do 
        {
            if (p.y() < m_targetRect.top()) {y = m_targetRect.top();break;}
            if (p.y() > m_targetRect.bottom()) {y=m_targetRect.bottom();break;}
            y = p.y();
        } while (0);
...
{
    // 如果不是画文本，则不判定有效区域
    if (m_Type!=drawText && (m_spBeginPoint.data()== nullptr || m_spEndPoint.data()== nullptr))
    {
        // 无效区域
        _drawState = drawInit;
        m_spBeginPoint.reset();
    ...
    if (m_Type==drawText && (!forcesave && m_spBeginPoint.data()== nullptr))
    {
        // 无效区域
        _drawState = drawInit;
        m_spBeginPoint.reset();
  ...
  QPixmap tempPixmap = m_lastPixmap;
    ...
    case drawText:
        {
            ...
            if (!pEdit->ShowOrHideEditWidget(m_targetRect, *m_spBeginPoint.data()))
            {
                ...
                rect.moveTopLeft(QPoint(rect.x()-m_targetRect.x()+2,rect.y()-m_targetRect.y() +2)); // 边框有1像素，修一下
    ...
    case  drawPen:
        {
           ...
           // rect.moveTopLeft(QPoint(rect.x()-m_targetRect.x(),rect.y()-m_targetRect.y()));
            ...
            for (int i=0;i<m_lstPenPoints.size()-1;i++)
            {
                ...
                p1 = QPoint(p1.x()-m_targetRect.x(),p1.y()-m_targetRect.y());
                p2 = QPoint(p2.x()-m_targetRect.x(),p2.y()-m_targetRect.y());
    ...
    case drawArror:
        {
            QPainter p(&tempPixmap);
            QPointF pBegin = QPointF(m_spBeginPoint->x()-m_targetRect.x(),m_spBeginPoint->y()-m_targetRect.y());
            QPointF pEnd = QPointF(m_spEndPoint->x()-m_targetRect.x(),m_spEndPoint->y()-m_targetRect.y());
...
void ActionCommand::setColor(const QColor &color) {
    m_color = color;
    if(m_Type == drawText)
    {
        floatTextEdit* pEdit = SnipScreenTool::getInstance()->getTextEdit();
        pEdit->setColor(color);
    }
}